// gp-page-v2.7.2.js — ES5, Forix-like UI, dynamic ribbon, no pagination, MT->FL, no header focus outline
(function(){
  'use strict';
  ...
  function buildDisplayRows(rows, sessionCode, raceId){
    ...
    var isRace=(c==='COURSE'||c==='RACE');
    var isFastest=(c==='FL'||c==='MT'); // MT=Meilleur Tour => Fastest Laps

    // meilleur temps de référence (non-course)
    var bestMs=null;
    if(!isRace){
    ...
    for(var j=0;j<rows.length;j++){
      var r=rows[j], drvId=pick(r,['driver_id','DriverId','driverId']);
      ...
      if(/^Q[1-4]$/i.test(c)) laps=''; // qualifs: pas de laps
    ...
    // pour les essais et FL/qualifs: si pos manquante, recalcul par meilleur temps
    if(!isRace){
