...
(function(){
  ...
  function labelFor(code){
    ...
    var map={ 'COURSE':'Race', 'RACE':'Race', 'GRID':'Grid', 'FL':'FL', 'LPBLP':'LPbLP', 'TLAPS':'TLaps', 'RESUME':'Resume' };
    return map[m]||m;
  }
  ...
  function buildResumeModel(){
    ...
    for(var di=0; di<driversOrder.length; di++){
      ...
      obj['LEAD_laps'] = hasLeadLaps && leadLapsMap[id]? leadLapsMap[id] : null;
      obj['LEAD_km']   = hasLeadKm && leadKmMap[id]? leadKmMap[id] : null;
    ...
    if(hasLeadLaps) columns.push({key:'LEAD_laps', label:'Lead laps', group:'blue'});
    if(hasLeadKm)   columns.push({key:'LEAD_km',   label:'Lead km',   group:'blue'});
    ...
    var tops=[ ['TOP1_laps','Top1 Laps'],['TOP1_lt','TOP1 Lt'],
               ['TOP3_laps','Top3 Laps'],['TOP3_lt','TOP3 Lt'],
               ['TOP6_laps','Top6 Laps'],['TOP6_lt','TOP6 Lt'],
               ['TOP10_laps','Top10 Laps'],['TOP10_lt','TOP10 Lt'] ];
    for(var t=0;t<tops.length;t++){
      if(anyKeyHasData(tops[t][0])) columns.push({key:tops[t][0], label:tops[t][1], group:'purple'});
    }
    if(anyKeyHasData('NB_TOURS_WKND')) columns.push({key:'NB_TOURS_WKND', label:'Nb tours weekend', group:'purple'});

    var filtered=[]; // on retire colonnes vides (sauf 2 premières)
